from shipdesk.tiers import restocking_fee as m
...
def test_next_tier_and_distance():
    ...
    assert m.distance_to_next(0) == second.threshold + 1
    assert m.next_tier(m.TIERS[-1].threshold + 1) is None
    assert m.distance_to_next(m.TIERS[-1].threshold + 1) is None
    assert first.label == m.TIERS[0].label


def test_summarize_counts_every_value():
    values = [0, m.TIERS[1].threshold + 1, m.TIERS[-1].threshold + 1]
