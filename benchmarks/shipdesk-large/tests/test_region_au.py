from shipdesk.regions import au as m


def test_valid_and_invalid_postal():
    assert m.is_valid_postal("2000")
    assert not m.is_valid_postal("200")


def test_normalize_postal_strips_and_uppercases():
    assert m.normalize_postal("  2000  ") == m.normalize_postal("2000")
...
def test_address_layout_contains_parts():
    text = m.format_address("Ada", "1 Main St", "Town", "ST", "2000")
