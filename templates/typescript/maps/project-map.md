...
> Every non-ignored file and directory has one row below; `tests/test_maps.test.ts` fails when a row is missing or stale.
...
| `tests/test_maps.test.ts` | test | Fails when a non-ignored path lacks a map row or CLAUDE.md drifts from AGENTS.md. | - |
